...
async function triggerLazySimulation() {
    const now = Date.now();
    if (now - lastSimulationTime > 4000) {
        lastSimulationTime = now;
        runSinglePingSimulation().catch(e => {
            console.error('[LAZY SIMULATION EXCEPTION]', e.message);
        });
    }
}
...
app.get('/api/devices', async (req, res) => {
    try {
        // Trigger background telemetry simulation if more than 4 seconds elapsed
        triggerLazySimulation();
